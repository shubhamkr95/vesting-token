...
contract TokenVesting {
 ...
 function releaseVesting(address _beneficiary) external payable {
  ...
  require(
   msg.sender == vested[_beneficiary].beneficiary || msg.sender == admin,
   "This function is authorized to admin or _beneficiary"
  );

  require(
   block.timestamp >= vested[_beneficiary].cliffDuration,
  ...
  uint256 beneficiaryBalance = token.balanceOf(
   vested[_beneficiary].beneficiary
  );

  require(
   beneficiaryBalance <= vested[_beneficiary].amountAfterVesting,
   "Vested amount fully transfered"
  );
  ...
  returns (uint256)
 {
  ...
  uint256 totalMonthsLeft = (_duration.sub(block.timestamp) / 60 / 60 / 24) /
   30;
  ...
  if (totalMonthsLeft <= 0) {
   ...
   totalAmountNow = (totalTokenAmount.mul(_percentage) / 100) / totalMonthsLeft;
  }
